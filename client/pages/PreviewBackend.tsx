...
export default function PreviewBackend() {
  ...
  const displayName: string =
    apiTemplate?.name || apiTemplate?.Name || apiTemplate?.nameOfTemplate || "Template";
  ...
  const buildTemplateConfigForReceiverView = () => {
    ...
    return {
      ...
      documentVerification: {
        enabled: sections.some((s: any) => s.sectionType === "documents" && s.isActive !== false),
      ...
      biometricVerification: {
        enabled: sections.some((s: any) => s.sectionType === "biometrics" && s.isActive !== false),
      },
    };
  };
  ...
  return (
    <div className="min-h-screen bg-white font-roboto">
      <header className="h-11 px-4 flex items-center justify-between border-b border-[#DEDEDD] bg-white">
        ...
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-full bg-[#F65F7C] flex items-center justify-center">
            <span className="text-white text-xs font-medium leading-[10px] font-roboto">OS</span>
          </div>
        </div>
      </header>
      ...
      <div className="border-b border-[#DEDEDD] bg-white">
        <div className="h-[38px] px-4 flex items-center gap-2">
          <div className="flex items-center gap-2">
            <div className="flex h-8 items-center gap-1">
              ...
              <span className="text-xs text-[#505258] font-medium leading-3 font-roboto">Template</span>
            </div>
            <div className="flex h-8 items-center gap-2">
              <span className="text-xs text-[#505258] font-medium leading-3 font-roboto">/</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex h-8 items-center gap-1">
              <span className="text-xs text-[#505258] font-medium leading-3 font-roboto">Preview</span>
            </div>
          </div>
        </div>
        ...
        <div className="h-12 px-4 py-2 flex items-center justify-between">
          <div className="flex items-start gap-2">
            <div className="flex items-start gap-2">
              <button onClick={handleBack} className="w-7 h-7 p-2 flex items-center justify-center rounded-full bg-[#F1F2F4] hover:bg-gray-200 transition-colors">
                <ChevronLeft className="w-4 h-4 text-[#676879]" strokeWidth={2} />
              </button>
              <h1 className="text-xl font-bold text-[#172B4D] leading-[30px] font-roboto">{displayName}</h1>
            </div>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={handleSaveAndSendInvite} className="h-8 px-2 py-[9px] flex items-center gap-1 rounded border border-[#0073EA] bg-white hover:bg-blue-50 transition-colors">
              <Send className="w-4 h-4 text-[#0073EA]" strokeWidth={1.33} />
              <span className="text-[13px] font-medium text-[#0073EA] font-roboto">Save & Send Invite</span>
            </button>
            <button onClick={handleSave} className="h-8 px-2 py-[9px] flex items-center gap-1 rounded border border-[#0073EA] bg-[#0073EA] hover:bg-blue-700 transition-colors">
          ...
          <div className="flex items-center justify-center">
            <div className="flex items-center gap-8">
              <div className="flex flex-col items-center gap-1.5">
                ...
                <span className="text-[13px] font-medium text-[#172B4D] font-roboto">Form builder</span>
              </div>
              ...
              <div className="flex flex-col items-center gap-1.5">
                <div className="p-1.5 rounded-full border-2 border-[#0073EA]">
                  <div className="w-8 h-8 rounded-full bg-[#0073EA] flex items-center justify-center">
                    <span className="text-white text-base font-bold leading-4 font-roboto">2</span>
                  </div>
                </div>
                <span className="text-[13px] font-medium text-[#172B4D] font-roboto">Preview</span>
              </div>
            </div>
          </div>
          <button onClick={() => navigate(-1)} className="absolute left-8 flex items-center gap-1 rounded hover:bg-gray-50 transition-colors">
            <ChevronLeft className="w-4 h-4 text-[#676879]" strokeWidth={2} />
            <span className="text-[13px] font-medium text-[#505258] font-roboto">Previous</span>
          </button>
        </div>
      </div>
              ...
              <div className="flex flex-col gap-2">
                <div className="w-[308px] px-[26px] py-3 flex items-center gap-2.5 rounded bg-[#E6F1FD]">
                  <div className="flex-1 flex flex-col gap-2">
                    <div className="flex items-center gap-1">
                      <h3 className="w-[248px] text-sm font-bold text-[#292F4C] leading-[13px] font-roboto">Admin View</h3>
                    </div>
                    <div className="flex items-center gap-2">
                      <p className="flex-1 text-[13px] text-[#505258] leading-[18px] font-roboto">Lorem Ipsum is simply dummy text of the printing and typesetting industry.</p>
                    </div>
                  </div>
                </div>
                <div role="button" tabIndex={0} onClick={handleReceiverViewClick} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") handleReceiverViewClick(); }} className="w-[308px] px-[26px] py-3 flex items-center gap-2.5 rounded opacity-50 cursor-pointer hover:bg-blue-50">
                  <div className="flex-1 flex flex-col gap-2">
                    <div className="flex items-center gap-1">
                      <h3 className="w-[248px] text-sm font-bold text-[#292F4C] leading-[13px] font-roboto">Receiver's View</h3>
                    </div>
                    <div className="flex items-center gap-2">
                      <p className="flex-1 text-[13px] text-[#505258] leading-[18px] font-roboto">Lorem Ipsum is simply dummy text of the printing and typesetting industry.</p>
                    </div>
                ...
                <div className="p-0 pb-px pl-px pr-px flex flex-col w-full rounded border border-[#DEDEDD]">
                  <div className="px-2 py-4 flex flex-col items-center gap-2 w/full bg-white">
                    <div className="flex items-center gap-2 w-full pb-1">
                      ...
                      <h2 className="text-base font-bold text-[#172B4D] leading-3 font-roboto">{section.title}</h2>
                    </div>
                    <div className="flex items-center gap-2.5 w-full pl-7">
                      <p className="flex-1 text-[13px] text-[#172B4D] leading-5 font-roboto">{section.description}</p>
                    </div>
                  </div>
                  <div className="px-[34px] py-5 flex flex-col w-full border-t border-[#DEDEDD] bg-white">{section.component}</div>
                </div>
              </div>
            ))}
      ...
      <SendInviteDialog isOpen={showSendInviteDialog} onClose={() => setShowSendInviteDialog(false)} />
    </div>
  );
}
...
function PersonalInformationSection({
  ...
  const baseFields = [
    showBase.firstName && { id: "firstName", name: "First Name", placeholder: "Enter your first name" },
    showBase.lastName && { id: "lastName", name: "Last Name", placeholder: "Enter your last name" },
    showBase.email && { id: "email", name: "Email Address", placeholder: "Enter your email address" },
              ...
              <div key={field.id} className="flex flex-col flex-1">
                <div className="flex gap-2 w-full pb-2">
                  <div className="flex flex-col justify-center flex-1 h-2.5">
                    <span className="text-[13px] font-medium text-[#172B4D] leading-[18px] font-roboto">{field.name}</span>
                  </div>
                </div>
                <div className="h-[38px] px-3 py-[15px] flex items-center justify-between w-full rounded border border-[#C3C6D4] bg-white">
                  <div className="flex items-center gap-2 flex-1">
                    <span className="text-[13px] text-[#676879] leading-5 font-roboto">{field.placeholder}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="flex gap-2 w-full">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-2">
            <div className="flex flex-col justify-center w-full h-2.5">
              <span className="text-[13px] font-medium text-[#172B4D] leading-[18px] font-roboto">{title}</span>
            </div>
            <p className="w-full text-[13px] text-[#505258] leading-5 font-roboto">{desc}</p>
          </div>
        </div>
      </div>
    </div>
          ...
          <div className="flex flex-col items-center gap-4 flex-1">
            <div className="flex gap-6 w-full">
              <div className="flex flex-col gap-2 flex-1">
                <div className="flex items-center gap-2">
                  <h3 className="text-base font-bold text-[#172B4D] leading-3 font-roboto">User Upload Options</h3>
                </div>
                <div className="flex items-center gap-2 w-full">
                  <p className="flex-1 text-[13px] text-[#172B4D] leading-5 font-roboto">Selected upload methods for document submission.</p>
                </div>
              </div>
            </div>
            <div className="pt-6 pr-0 pb-0 pl-6 flex flex-col gap-5 w-full rounded bg-[#F6F7FB]">
              {config.allowUploadFromDevice && (
                <DocListItem title="Allow Upload from Device" desc="Let users upload existing documents directly from their device." />
              )}
              {config.allowCaptureWebcam && (
                <DocListItem title="Allow Capture via Webcam" desc="Enable webcam access to allow users to capture documents in real time." />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center w-full bg-white">
          <div className="flex flex-col items-center gap-4 flex-1">
            <div className="flex gap-6 w-full">
              <div className="flex flex-col gap-2 flex-1">
                <div className="flex items-center gap-2">
                  <h3 className="text-base font-bold text-[#172B4D] leading-3 font-roboto">Unreadable Document Handling</h3>
                </div>
                <div className="flex items-center gap-2 w-full">
                  <p className="flex-1 text-[13px] text-[#172B4D] leading-5 font-roboto">Action taken if submitted document is unclear.</p>
                </div>
              </div>
            </div>
            <div className="pt-6 pr-0 pb-0 pl-6 flex flex-col gap-5 w-full rounded bg-[#F6F7FB]">
              <div className="h-[58px] pb-5 flex flex-col w-full">
                <div className="pb-5 flex flex-col w-full">
                  <div className="flex gap-2 w-full">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col gap-2">
                      <div className="flex flex-col justify-center w-full h-2.5">
                        <span className="text-[13px] font-medium text-[#172B4D] leading-[18px] font-roboto">{config.documentHandling === "retry" ? "Allow Retries Before Rejection" : "Reject Immediately"}</span>
                      </div>
                      <p className="w-full text-[13px] text-[#505258] leading-5 font-roboto">{config.documentHandling === "retry" ? "Let users reattempt uploading the document before it's finally rejected." : "Skip retry and reject unclear documents without further attempts."}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {config.selectedDocuments && config.selectedDocuments.length > 0 && (
        <div className="flex flex-col items-center gap-4 w-full">
          <div className="flex gap-6 w-full">
            <div className="flex flex-col gap-2 flex-1">
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-[#172B4D] leading-3 font-roboto">Supported Documents for Identity Verification</h3>
              </div>
              <div className="flex items-center gap-2 w-full">
                <p className="flex-1 text-[13px] text-[#172B4D] leading-5 font-roboto">Only these document types are accepted.</p>
              </div>
            </div>
          </div>
          <div className="h-[165px] pt-6 px-6 pb-0 flex flex-col gap-2 w-full rounded bg-[#F6F7FB]">
            <div className="px-3 pb-3 flex flex-col w-full rounded-lg bg-white">
              <div className="h-[42px] flex items-center gap-6 w-full">
                <span className="text-sm font-medium text-black leading-[22px] font-roboto">India</span>
              </div>
      ...
      <div className="flex items-center w-full rounded-t-lg bg-white">
        <div className="flex flex-col items-center gap-4 flex-1">
          <div className="flex gap-6 w-full">
            <div className="flex flex-col gap-2 flex-1">
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-[#172B4D] leading-3 font-roboto">Biometric Settings</h3>
              </div>
              <div className="flex items-center gap-2 w-full">
                <p className="flex-1 text-[13px] text-[#172B4D] leading-5 font-roboto">Configured liveness and retry policy.</p>
              </div>
            </div>
          </div>
          <div className="pt-6 pr-0 pb-0 pl-6 flex flex-col gap-5 w-full rounded bg-[#F6F7FB]">
            <DocListItem title={`Max Retries: ${config?.maxRetries ?? "Not set"}`} desc="Number of allowed retry attempts for liveness." />
            {config?.askUserRetry && (
              <DocListItem title="Ask User to Retry" desc="Prompt users to try again if liveness fails." />
            )}
            {config?.blockAfterRetries && (
              <DocListItem title="Block After Retries" desc="Block further attempts after exceeding retries." />
            )}
            {config?.dataRetention && (
              <DocListItem title={`Data Retention: ${config.dataRetention}`} desc="Retention policy for biometric data." />
            )}
          </div>
        </div>
      </div>
