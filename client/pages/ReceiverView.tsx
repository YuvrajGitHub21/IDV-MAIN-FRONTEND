...
type SectionKey = "Personal_info" | "Doc_verification" | "Biometric_verification";
...
interface FormData {
  ...
  // dynamic optional personal fields from DB:
...
/* ----------------------------- Normalizers & Mappers ----------------------------- */
const yes = (v: any) => v === true || v === "true" || v === 1;

function normalizeStatus(status: any = {}) {
  const s = status || {};
  return {
    personal_info:
      s.personal_info ?? s.persoanl_info ?? s.persoanlInfo ?? s.Personal_info ?? true,
    doc_verification:
      s.doc_verification ?? s.Doc_verification ?? s.document_verification ?? false,
    biometric_verification:
      s.biometric_verification ?? s.Biometric_verification ?? false,
  };
}

function normalizeKey(k: string): SectionKey | null {
  const t = (k || "").toLowerCase().replace(/\s+/g, "_");
  if (t.includes("personal")) return "Personal_info";
  if (t.includes("doc")) return "Doc_verification";
  if (t.includes("biometric")) return "Biometric_verification";
  return null;
}

function getPersonalShowBase(tpl: any) {
  const p = tpl?.Personal_info || {};
  return {
    firstName: yes(p.firstName ?? p.FirstName),
    lastName: yes(p.lastName ?? p.LastName),
    email: yes(p.email ?? p.Email),
  };
}

function getPersonalAddedFields(tpl: any) {
  const a = tpl?.Personal_info?.Added_fields || {};
  const out: Array<{ id: string; label: string; type?: string; placeholder?: string }> = [];
  if (a.dob) out.push({ id: "dateOfBirth", label: "Date Of Birth", type: "date", placeholder: "" });
  if (a.Current_address)
    out.push({ id: "currentAddress", label: "Current Address", placeholder: "Enter your current address" });
  if (a.permanent_address)
    out.push({ id: "permanentAddress", label: "Permanent Address", placeholder: "Enter your permanent address" });
  if (a.Gender) out.push({ id: "gender", label: "Gender", placeholder: "Select gender" });
  return out;
}

function getDocConfigFromDb(tpl: any) {
...
export default function ReceiverView() {
  ...
  // Derived config from DB
  const sectionStatus = dbTemplate ? normalizeStatus(dbTemplate.Section_status) : null;

  const showPersonal = sectionStatus ? !!sectionStatus.personal_info : true;
  const showDoc = sectionStatus ? !!sectionStatus.doc_verification : !!dbTemplate?.Doc_verification;
  const showBio = sectionStatus ? !!sectionStatus.biometric_verification : !!dbTemplate?.Biometric_verification;

  // Respect DB section order
  const DEFAULT_ORDER: SectionKey[] = ["Personal_info", "Doc_verification", "Biometric_verification"];
  const rawOrder: string[] =
    Array.isArray(dbTemplate?.sections_order) && dbTemplate.sections_order.length
      ? dbTemplate.sections_order
      : DEFAULT_ORDER;
  const orderedKeys = (rawOrder.map(normalizeKey).filter(Boolean) as SectionKey[]).length
    ? (rawOrder.map(normalizeKey).filter(Boolean) as SectionKey[])
    : DEFAULT_ORDER;

  // Personal fields from DB
  const baseShow = dbTemplate
    ? getPersonalShowBase(dbTemplate)
    : { firstName: true, lastName: true, email: true };
  const addedPersonal = dbTemplate ? getPersonalAddedFields(dbTemplate) : [];

  // Document config from DB or fallback to localStorage
  const docConfig = useMemo(() => {
    if (dbTemplate) return getDocConfigFromDb(dbTemplate);
    try {
  ...
  // Mapping & filtering of ID types based on DB selection
  ...
  const handleSubmit = () => {
    ...
    if (showPersonal) {
      if (baseShow.firstName && !formData.firstName?.trim()) newErrors.firstName = "First name is required";
      if (baseShow.lastName && !formData.lastName?.trim()) newErrors.lastName = "Last name is required";
      if (baseShow.email) {
        if (!formData.email?.trim()) newErrors.email = "Email is required";
        else if (!emailVerified) newErrors.email = "Email verification is required";
      }
      if (addedPersonal.some((f) => f.id === "dateOfBirth") && !formData.dateOfBirth) {
        newErrors.dateOfBirth = "Date of birth is required";
      }
      // Add other added fields as required if your product says so
      // e.g., if currentAddress is required when enabled:
      // if (addedPersonal.some((f) => f.id === "currentAddress") && !formData.currentAddress?.trim()) { ... }
    }

    if (showDoc) {
      if (!formData.idType) newErrors.idType = "Please select an ID type";
      // If you want to require document upload when any method is available:
      // if ((allowUploadFromDevice || allowCaptureWebcam) && !formData.document) {
      //   newErrors.document = "Please upload or capture your document";
      // }
    }

    // If you have biometric inputs to validate, add them here when enabled:
    // if (showBio) { ... }
        ...
        <CardContent className="p-8 space-y-6">
          ...
          {baseShow.email && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Date of Birth only if enabled via Added_fields */}
              {addedPersonal.some((f) => f.id === "dateOfBirth") && (
                <div className="space-y-2">
          ...
          {/* Other DB-added personal fields */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {addedPersonal
              .filter((f) => f.id !== "dateOfBirth")
              .map((f) => (
                <div key={f.id} className="space-y-2">
                  <Label className="text-[13px] font-medium text-[#172B4D]">{f.label}</Label>
                  <Input
    ...
    return (
      <Card className="border border-[#DEDEDD]">
        <div className="p-4 bg-white border-b border-[#DEDEDD]">
          ...
          <p className="text-[13px] text-[#172B4D] ml-6">
            Choose a valid government-issued ID (like a passport, driver's license, or national ID) and upload a clear
            photo of it.
          </p>
        </div>

        <CardContent className="p-8 space-y-6">
          {/* Country Selection (kept generic) */}
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Extra dropdown kept; replace with anything needed */}
              <Select>
                <SelectTrigger className="h-[38px] border-[#C3C6D4] text-[13px]">
          ...
          {/* ID Type Selection - filtered by DB */}
          ...
          {/* Upload Methods - only those allowed by DB */}
                  ...
                  >
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-[13px] text-[#676879]">
                        Upload a photo or PDF of your document from this device.
                      </p>
                    </div>
                    <input
                      ref={fileInputRef}
          ...
          {showCamera && (
            <div className="grid grid-cols-1 gap-2">
              <div className="border-2 border-dashed border-[#C3C6D4] rounded-lg p-4 flex flex-col items-center gap-4 min-h-[320px] justify-center relative">
                ...
                {capturedDataUrl && <img src={capturedDataUrl} alt="Captured" className="w-full max-w-md rounded" />}
                {cameraError && (
    ...
    return (
      <Card className="border border-[#DEDEDD]">
        <div className="p-4 bg-white border-b border-[#DEDEDD]">
          ...
          <p className="text-[13px] text-[#172B4D] ml-6">
            Take a live selfie to confirm you are the person in the ID document. Make sure you're in a well-lit area and
        ...
        <CardContent className="p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Camera Section (placeholder UI kept) */}
            <div className="space-y-4">
  ...
  return (
    <div className="min-h-screen bg-white font-roboto">
      ...
      {tplError && (
        <div className="px-4 py-2 text-sm text-red-700 bg-red-50 border-b border-red-200">{tplError}</div>
      )}
        ...
        <div className="h-12 px-4 py-2 flex items-center justify-between">
          <div className="flex items-start gap-2">
            ...
            <h1 className="text-xl font-bold text-[#172B4D] leading-[30px]">
              {dbTemplate?.nameOfTemplate ?? "New Template"}
            </h1>
          </div>
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={() => {
                ...
                showSaveSuccessToast(dbTemplate?.nameOfTemplate ?? "New Template");
                navigate("/dashboard");
              }}
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="w-[332px] bg-white flex flex-col border-r border-[#DEDEDD]">
          <div className="p-4 flex flex-col gap-4">
            {/* Admin View Tab - Inactive (clickable) */}
            <div
              role="button"
              tabIndex={0}
              onClick={() =>
            ...
            <div className="w-full px-[26px] py-3 flex items-center gap-2.5 rounded bg-[#E6F1FD]">
              <div className="flex-1 flex flex-col gap-2">
                ...
                <p className="text-[13px] text-[#505258] leading-[18px]">Fill the form as the end user.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          <div className="max-w-4xl mx-auto space-y-6">
            {/* Render sections in DB-defined order */}
            {orderedKeys.map((key) => {
              if (key === "Personal_info") return <div key={key}>{renderPersonalSection()}</div>;
              if (key === "Doc_verification") return <div key={key}>{renderDocSection()}</div>;
              if (key === "Biometric_verification") return <div key={key}>{renderBiometricSection()}</div>;
              return null;
            })}
